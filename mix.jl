...
function solveMixed01WithoutCuts(m::Int, epsilon::Float64)
    ...
    #"CPXPARAM_Preprocessing_Presolve" => 0 , "CPX_PARAM_MIPSEARCH" => 1 , "CPX_PARAM_MIRCUTS" => -1 , "CPX_PARAM_FRACCUTS" => -1 , "CPX_PARAM_FLOWCOVERS" => -1
    ...
    @constraint(model, c^2 >= ((1-epsilon)/epsilon) * ((sum(sigma[i]^2 * x[i]^2 for i in (1:n)')) + (sum(sigma[i+n]^2 * y[i]^2 for i in (1:m)'))))
